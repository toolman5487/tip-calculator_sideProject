//
//  AppIndicatorFilterHeaderView.swift
//  tip-calculator
//
...
final class AppIndicatorFilterHeaderView: UICollectionReusableView {

    static let reuseId = "AppIndicatorFilterHeaderView"
    private var viewModel: AppIndicatorFilterHeaderViewModel?
    ...
    private lazy var horizontalCollectionView: UICollectionView = {
        ...
        cv.delegate = self
        cv.dataSource = self
        cv.register(IndicatorCapsuleCell.self, forCellWithReuseIdentifier: IndicatorCapsuleCell.reuseId)
        return cv
    }()
    ...
    func configure(with viewModel: AppIndicatorFilterHeaderViewModel) {
...
extension AppIndicatorFilterHeaderView: UICollectionViewDataSource {
    func collectionView(_ collectionView: UICollectionView, numberOfItemsInSection section: Int) -> Int {
        viewModel?.options.count ?? 0
    }

    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        let cell = collectionView.dequeueReusableCell(withReuseIdentifier: IndicatorCapsuleCell.reuseId, for: indexPath) as! IndicatorCapsuleCell
...
extension AppIndicatorFilterHeaderView: UICollectionViewDelegateFlowLayout {
